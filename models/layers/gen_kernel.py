...
class GenerateKernelLayer21(nn.Module):
    def __init__(self, stages_channel, kernel_size=3, hidden_channels=256):
        ...
        for i, c in enumerate(self.stages_channel):
        ...
        # transformer
        # self.decoder_wrapper = TransDecoderWrapperLayer2(None, hidden_channels)
        # self.position_embed = PositionEmbeddingSine(hidden_channels // 2)
        # self.query_embed = nn.Embedding(9, hidden_channels)

        # head
        self.head_block = self.make_head_layer(self.dilation_num, self.dilation_num)
        self.head_block_copy = self.make_head_layer(self.dilation_num, self.dilation_num)
        ...
        for i, kernel in enumerate(self.kernel_list):
            ...
            for j in range(0, kernel.shape[0]):
                ...
                for k, extra_kenel_k in enumerate(extra_kernels):
                    ...
                    extra_sim_map = F.conv2d(single_map, extra_single_kernel, padding=(kernel.shape[-2] // 2, kernel.shape[-1] // 2))
            ...
            sim_map = self.head_block(sim_map) if level == 3 else self.head_block_copy(sim_map)
    ...
    def make_kernel_extractor(self, hidden_channels, dilation_num = 3):
        ...
        # extract kernerls
        conv1 = self.make_conv_layer(hidden_channels, 3, 2, p1=1)  # 32*32
        conv2 = self.make_conv_layer(hidden_channels, 3, 2, p1=1)  # 16*16
        conv3 = self.make_conv_layer(hidden_channels, 3, 2, p1=1)  # 8*8
        conv4 = self.make_conv_layer(hidden_channels, 3, 2)  # 3*3
        # conv5 = self.make_conv_layer(hidden_channels, 3, 1)  # 3*3

        # conv6 = self.make_conv_layer(hidden_channels, 3, 1, p1=1, p2=0)  # 1*1
        kernel_blocks = [conv1, conv2, conv3, conv4]
